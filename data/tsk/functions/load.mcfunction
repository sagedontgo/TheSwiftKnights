scoreboard objectives add tsk.actionbarToggle trigger
scoreboard objectives add tsk.actionbarSwitch dummy
scoreboard objectives add tsk.actionbarTimer dummy
scoreboard objectives add tsk.leftClick dummy
scoreboard objectives add tsk.rightClick dummy
scoreboard objectives add tsk.leftClick.timer dummy
scoreboard objectives add tsk.rightClick.timer dummy

scoreboard objectives add tsk.mana dummy
scoreboard objectives add tsk.manaMax dummy
scoreboard objectives add tsk.manaRegen dummy
scoreboard objectives add tsk.manaRegenRate dummy
scoreboard objectives add tsk.ultPercentage dummy
scoreboard objectives add tsk.ultRegenerate dummy
scoreboard objectives add tsk.ultPercentageMax dummy

# You will be useful eventually
scoreboard objectives add tsk.currentLevel dummy
scoreboard objectives add tsk.nextLevel dummy
scoreboard objectives add tsk.levelProgress dummy
scoreboard objectives add tsk.rolledLevelProgress dummy

scoreboard objectives add tsk.id dummy

scoreboard objectives add tsk.raySteps dummy
scoreboard objectives add tsk.raySuccess dummy

# Timer for each abilities to prevent conflicts i think
scoreboard objectives add tsk.icefrozenground.timer dummy
scoreboard objectives add tsk.mermaidssong.timer dummy

team add tsk.hostiles
team modify tsk.hostiles seeFriendlyInvisibles false
team modify tsk.hostiles friendlyFire false