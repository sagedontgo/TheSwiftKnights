...
scoreboard players operation $temp.tier3.mobLevel tsk.calcNewHp = @s tsk.mobLevel

execute store result score $tier3.temp.lvl tsk.calcNewHp run scoreboard players operation $temp.tier3.mobLevel tsk.calcNewHp *= $10000 tsk.int
...
execute store result score $tier3.temp.lvl tsk.calcNewHp run scoreboard players operation $temp.tier3.mobLevel tsk.calcNewHp /= $2 tsk.int

execute store result score $tier3.temp.z tsk.calcNewHp run scoreboard players operation $tier3.temp.z tsk.calcNewHp += $tier3.temp.lvl tsk.calcNewHp
...
scoreboard players reset $temp.tier3.mobLevel tsk.calcNewHp
scoreboard players reset $temp.worldLevel tsk.calcNewHp
scoreboard players reset $tier3.temp.lvl tsk.calcNewHp
